...
export type LatLonTupleToXYTuple = (latLon: [number, number]) => [number, number]
export type XYTupleToLatLonTuple = (xy: [number, number]) => [number, number]
...
export function useLatLonToXy(zoom = 5000, width: number) {
  const {
    ...
    const xYTupleToLatLonTuple = (xy: [number, number]): [number, number] => {
      const [x, y] = xy
